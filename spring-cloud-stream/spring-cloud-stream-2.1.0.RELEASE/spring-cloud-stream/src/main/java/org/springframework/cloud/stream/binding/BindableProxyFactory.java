...
public class BindableProxyFactory implements MethodInterceptor, FactoryBean<Object>, Bindable, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
            ...
            public void doWith(Method method) throws IllegalArgumentException {
                ...
                if (input != null) {
                    ...
                    if (sharedBindingTarget != null) {
                        ...
                        // 根据返回值匹配合适的 BindingTargetFactory，匹配不到或者匹配到多个均会抛异常
                        BindingTargetFactory bindingTargetFactory = getBindingTargetFactory(returnType);
                        Object boundTarget = bindingTargetFactory.createInput(name);
                        BindableProxyFactory.this.inputHolders.put(name, new BoundTargetHolder(boundTarget, true));
                    }
                }
            }
        });
        ReflectionUtils.doWithMethods(this.type, new ReflectionUtils.MethodCallback() {
            ...
            public void doWith(Method method) throws IllegalArgumentException {
                Output output = AnnotationUtils.findAnnotation(method, Output.class);
                if (output != null) {
                    // 获取 @Output 注解的value值，如果没有配置，则取所在方法名
                    String name = BindingBeanDefinitionRegistryUtils.getBindingTargetName(output, method);
                    Class<?> returnType = method.getReturnType();
                    Object sharedBindingTarget = locateSharedBindingTarget(name, returnType);
                    if (sharedBindingTarget != null) {
                        ...
                        // 根据返回值匹配合适的 BindingTargetFactory，匹配不到或者匹配到多个均会抛异常
                        BindingTargetFactory bindingTargetFactory = getBindingTargetFactory(returnType);
                        Object boundTarget = bindingTargetFactory.createOutput(name);
                        BindableProxyFactory.this.outputHolders.put(name, new BoundTargetHolder(boundTarget, true));
                    }
                }
            }
        });
    ...
    public Collection<Binding<Object>> createAndBindInputs(BindingService bindingService) {
        ...
        for (Map.Entry<String, BoundTargetHolder> boundTargetHolderEntry : this.inputHolders.entrySet()) {
            ...
            if (boundTargetHolder.isBindable()) {
                ...
                Object boundTarget = boundTargetHolder.getBoundTarget();
                Collection<Binding<Object>> consumers = bindingService.bindConsumer(boundTarget, inputTargetName);
                bindings.addAll(consumers);
            }
        }
        return bindings;
    }
    ...
    public Collection<Binding<Object>> createAndBindOutputs(BindingService bindingService) {
        ...
        /**
         * outputHolders缓存的是 @EnableBinding 注解中value属性对应接口的 @Output 的方法
         */
        for (Map.Entry<String, BoundTargetHolder> boundTargetHolderEntry : this.outputHolders.entrySet()) {
            ...
            if (boundTargetHolderEntry.getValue().isBindable()) {
                ...
                Object boundTarget = boundTargetHolder.getBoundTarget();
                Binding<Object> producer = bindingService.bindProducer(boundTarget, outputTargetName);
                bindings.add(producer);
            }
        }
        return bindings;
    }
