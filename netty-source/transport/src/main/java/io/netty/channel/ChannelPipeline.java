 ...
 *  *
 *  *       它里面封装的是一个ChannelHandler的集合, 她会拦截所有的进来出去的集合.它实现了高级过滤器模式,
 *  *       他给用户完全的控制事件的能力,控制这个管道中每一个handler之间协调合作
 *  *
 *  * <h3>Creation of a pipeline</h3>
 *  *
 *  * Each channel has its own pipeline and it is created automatically when a new channel is created.
 *  *       每一个通道都有他自己的管道,并且当这个通道被创建的时候,这个管道也会自动的创建(在AbstractChannel中创建出来的)
 ...
 *
 *      下面这张图描述了 IO事件 在channelPipeline中是如何被处理的, 会被ChannelInboundHandler(进来的)  ChannelOutboundHandler(出去的) 这两个处理器之一处理
 *      处理之后会调用 事件的传播方法propagation , 把事件传播给与之最近的处理器, 这个传播方法被定义在 ChannelHandlerContext ,比如说 fireChannelRead();
 * <pre>
 *
 *      netty中,把处理器分成了入栈和出栈两种, 只有同时是入in /出out 栈的处理器,请求才会同时经过他们
 *
 *                                                 I/O Request
 *                                            via {@link Channel} or
 ...
 *      入栈数据通常是由远端的一个实际的操作读取过来的, 比如 SocketChannel 的 read(ByteBuffer)
 * If an inbound event goes beyond the top inbound handler, it is discarded
 * silently, or logged if it needs your attention.
 *      如果这个入栈的数据已经到达上图的顶部, !!!!---  入栈数据就被悄无声息的丢弃掉,---- !!!! 如果需要我们可以选择用日志的方式记录下它
 * <p>
 *
 * An outbound event is handled by the outbound handler in the top-down direction as shown on the right side of the
 * diagram.  An outbound handler usually generates or transforms the outbound traffic such as write requests.
 *      出栈处理器,处理的一些被写出去的事件数据, 比如写请求
 *
 ...
 * <p>
 *      如果出栈的事件到达了处理器的底部, 和IO相关的线程就会执行实实在在的写操作, 比如 SocketCHannel的 write(ByteBuffer)
 *
 ...
 * 一个用户 在一个pipeline中 可能有一个或者多个 ChannelHandler 去接收用户的IO事件, 或者处理读写操作
 *
 * For example, a typical server will have the following handlers
 * in each channel's pipeline, but your mileage may vary depending on the complexity and characteristics of the
 * protocol and business logic:
 * 一个典型的服务器在每一个通道的pipeline中, 会有如下的 handler, 但是你的想法可能会根据协议的复杂性而有所区别
 *
 *
 * <ol>
 ...
 * pipeline, 去运行  MyBusinessLogicHandler 处理器中指定的方法
 ...
 * 这个方法很重要, 它指明了  new MyBusinessLogicHandler()里面的方法的回调是由 group里面的线程执行的,而不是由IO线程执行的
 * 目的就是不要让 耗时的任务阻塞IO线程, 他算是netty提供的标准的方式
 * 另一种解决办法,就是我们 手动使用线程池提交给他这个耗时的任务,让他去执行
 * </pre>
 *
 * <h3>Thread safety</h3>
 * <p>
 * A {@link ChannelHandler} can be added or removed at any time because a {@link ChannelPipeline} is thread safe.
 * ChannelHandler 在任何时候都可以被删除添加, 因为channelPipeline 本身就是线程安全的
 * For example, you can insert an encryption handler when sensitive information is about to be exchanged, and remove it
 * after the exchange.
 */


// 直观的看上去，下面的全都是针对 handler 的 CRUD操作
public interface ChannelPipeline
     ...
     *
     * 将ChannelHandler 插入到 ChannelPipeline的最后一个参数上   继续进去
     */
