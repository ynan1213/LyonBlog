...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService
{
    ...
    public void processConsumeResult(final ConsumeConcurrentlyStatus status, final ConsumeConcurrentlyContext context, final ConsumeRequest consumeRequest)
    {
        ...
        {
            case BROADCASTING:
                // 下面的for循环，只有在前面返回了RECONSUME_LATER才会进入
                // 无论消息消费返回是SUCCESS还是 RECONSUME_LATER，消息都不会被重新消费
            ...
            case CLUSTERING:
                ...
                // 如果是 SUCCESS，上面设置的ackIndex = getMsgs().size() - 1，所以这里不会进入for循环，也就不会执行 sendMessageBack
                ...
                for (int i = ackIndex + 1; i < consumeRequest.getMsgs().size(); i++)
                {
                    ...
                    if (!result)
                    {
                        // 如果 sendMessageBack 失败，加入失败集合，下面会将这些消息延迟5s后再次进行消费
