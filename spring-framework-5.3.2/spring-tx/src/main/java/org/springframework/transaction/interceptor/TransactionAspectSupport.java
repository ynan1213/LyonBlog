...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	public void afterPropertiesSet() {
		if (getTransactionManager() == null && this.beanFactory == null) {
			throw new IllegalStateException(
					"Set the 'transactionManager' property or make sure to run within a BeanFactory containing a TransactionManager bean!");
		}
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass, final InvocationCallback invocation) throws Throwable {
		...
		//获取事务属性解析器，用于解析目标方法以及类上@Transactional注解的属性，内部会缓存解析的结果，下次可以直接获取
		TransactionAttributeSource tas = getTransactionAttributeSource();
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);

		/**
		 * 获取平台事物管理器：
		 * 	1.如果@Transactional注解的属性指定了transactionManager，则从容器获取指定的的管理器
		 *  2.如果注解没有指定，从容器获取TransactionManager类型的bean，如果有多个会报错
		 */
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm, @Nullable TransactionAttribute txAttr, final String joinpointIdentification) {
		...
		// 封装 txAttr、TransactionStatus为 TransactionInfo 并绑定到 ThreadLocal
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			//根据异常类型判断是否回滚，可以通过Transactional注解中 rollbackFor、noRollbackFor、rollbackForClassName、noRollbackForClassName 配置
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
				...
				// 异常类型不满足，说明不回滚，在这里提交事物，如果非边界事物，也不会真正提交事物
				try {
					txInfo.getTransactionManager().commit(txInfo.getTransactionStatus());
				}
				catch (TransactionSystemException ex2) {
