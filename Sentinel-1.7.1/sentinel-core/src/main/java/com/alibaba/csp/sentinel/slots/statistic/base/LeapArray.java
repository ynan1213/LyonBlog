...
public abstract class LeapArray<T> {
    ...
    // 实际存储内容的数组，时间窗格个数在构造器内就会初始化完成，一个窗格存储一个WindowWrap对象
    // 默认长度为2
    ...
    public WindowWrap<T> currentWindow(long timeMillis) {
        ...
        // 计算当前时间落在哪个时间窗格，如果1s分为两个时间窗格，那么当前时间要么落在第一个要么落在第二个
        ...
        while (true) {
            ...
            if (old == null) {
                 ...
                 *
                 * 如果当前时间窗格内容为空，则创建一个，注意最重要的属性 windowStart 当前时间窗格的起始时间
                 */
                ...
                // 如果当前窗口的开始时间time小于old的开始时间，只有出现了时间回拨才会进入这里
                // 这里是仍返回了一个时间窗格，但是是new出来的，后面统计也不会统计里面的内容
