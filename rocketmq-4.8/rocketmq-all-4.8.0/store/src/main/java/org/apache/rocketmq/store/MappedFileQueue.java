...
public class MappedFileQueue
{
    ...
    // 当前刷盘指针，表示该指针之前的所有数据已全部持久化到磁盘
    private long flushedWhere = 0;

    // 当前数据提交指针，内存中ByteBuffer当前的写指针，该值大于或等于flushedWhere
    ...
    // 从MappedFile列表的第一个文件开始查找，找到第一个最后一次更新时间大于待查找时间戳的文件
    ...
    public boolean load()
    {
        //加载 ${ROCKET_HOME}\store\commitlog 下的所有文件
        ...
        if (files != null)
        {
            ...
            for (File file : files)
            {
                //如果文件大小和配置的单个文件大小不一致，将忽略该目录下的所有文件
                ...
                try
                {
                    ...
                    // wrotePostion、flushedPostion、committedPostion 三个指针都被设置为文件大小 ？？？
            ...
            if (firstMappedFile != null && lastMappedFile != null)
            {
                if (offset < firstMappedFile.getFileFromOffset() || offset >= lastMappedFile.getFileFromOffset() + this.mappedFileSize)
                {
                    LOG_ERROR.warn("Offset not matched. Request offset: {}, firstOffset: {}, lastOffset: {}, mappedFileSize: {}, mappedFiles count: {}",
                            offset, firstMappedFile.getFileFromOffset(), lastMappedFile.getFileFromOffset() + this.mappedFileSize,
                            this.mappedFileSize, this.mappedFiles.size());
                } else
                {
                    // 为什么不是直接 offset / this.mappedFileSize 还要减去 firstMappedFile.getFileFromOffset() / this.mappedFileSize呢？
                    // 因为有可能第一个文件被删除导致偏移量并不是从0开始
