...
/**
 * A {@link ChannelOption} allows to configure a {@link ChannelConfig} in a type-safe  todo 进入ChannelConfig
 * way.
 *      让我们以一种 类型安全的模式 配置ChannelConfig
 *
 * Which {@link ChannelOption} is supported depends on the actual implementation
 * of {@link ChannelConfig} and may depend on the nature of the transport it belongs
 * to.
 *      具体是哪一种 ChannelOption 取决于 实际实现的是哪一种 ChannelConfig, 也取决于它依赖的传输层
 *
 * @param <T>   the type of the value which is valid for the {@link ChannelOption}
 *
 *
 *  主要是使用它进行TCP底层的设置项, 去完成设定
 *  表示设置项对应的值的类型 , 进入它的父类
 *  ChannelOption 本身只维护key , 而值并不是他自己维护的! 它的Key 可以起到校验的 作用,
 *           比如, T == Boolean, 那么获取出来的ChannelOnption的常量类型为布尔类型
 */

public class ChannelOption<T> extends AbstractConstant<ChannelOption<T>> {

    // 下面这段代码是在 ChannelOption 中创建出常量池ConstantPool, 在这个常量池中维护这具体的 ChannelOption的信息
    ...
    //关闭连接时，允许半关，默认不允许；
    // 半关之后，Server 还是可以向 Client 发数据，只是不能从 Client 再读数据，Client 也不能向 Server 发数据了；
