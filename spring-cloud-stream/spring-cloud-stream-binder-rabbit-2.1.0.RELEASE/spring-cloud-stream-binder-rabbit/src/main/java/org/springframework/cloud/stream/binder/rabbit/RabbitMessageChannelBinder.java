...
public class RabbitMessageChannelBinder
    ...
    protected MessageHandler createProducerMessageHandler(
        ...
        RabbitTemplate rabbitTemplate = buildRabbitTemplate(producerProperties.getExtension(), errorChannel != null);
        ...
        if (errorChannel != null) {
            ...
            if (!ackChannelBeanName.equals(IntegrationContextUtils.NULL_CHANNEL_BEAN_NAME) && !getApplicationContext()
                .containsBean(ackChannelBeanName)) {
    ...
    private boolean expressionInterceptorNeeded(RabbitProducerProperties extendedProperties) {
        return extendedProperties.getRoutingKeyExpression() != null && extendedProperties.getRoutingKeyExpression().getExpressionString()
            .contains("payload")
            || (extendedProperties.getDelayExpression() != null && extendedProperties.getDelayExpression().getExpressionString()
            .contains("payload"));
    }
    ...
    private void checkConnectionFactoryIsErrorCapable() {
        if (!(this.connectionFactory instanceof CachingConnectionFactory)) {
            ...
            if (!ccf.isPublisherConfirms() && !ccf.isPublisherReturns()) {
                logger.warn(
                    "Producer error channel is enabled, but the connection factory is not configured for returns or confirms; the error channel will receive no messages");
            } else if (!ccf.isPublisherConfirms()) {
                logger.info(
                    "Producer error channel is enabled, but the connection factory is only configured to handle returned messages; negative acks will not be reported");
            } else if (!ccf.isPublisherReturns()) {
                logger.info(
                    "Producer error channel is enabled, but the connection factory is only configured to handle negatively acked messages; returned messages will not be reported");
            }
        }
    }
            ...
            return new MessageHandler() {
                ...
                public void handleMessage(org.springframework.messaging.Message<?> message) throws MessagingException {
                    ...
                    if (!(message instanceof ErrorMessage)) {
                        ...
                        this.template.send(this.exchange, this.routingKey != null ? this.routingKey : messageProperties.getConsumerQueue(),
                            amqpMessage);
                    }
                }
    ...
    private RabbitTemplate buildRabbitTemplate(RabbitProducerProperties properties, boolean mandatory) {
        ...
        if (properties.isBatchingEnabled()) {
            ...
            rabbitTemplate = new BatchingRabbitTemplate(batchingStrategy, getApplicationContext().getBean(IntegrationContextUtils.TASK_SCHEDULER_BEAN_NAME, TaskScheduler.class));
        } else {
