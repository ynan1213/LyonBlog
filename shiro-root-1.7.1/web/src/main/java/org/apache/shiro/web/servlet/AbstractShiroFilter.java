...
public abstract class AbstractShiroFilter extends OncePerRequestFilter
{
     ...
     * @since 1.0
     *
     * isHttpSessions()决定是否是httpSession，即原生session
     * 如果是，调用`subject.getSession()`和`ShiroHttpServletRequest.getSession()`方法最终调用的是原生`request.getSession(false)`
     * 如果不是，一般是通过cookie、uri和requestParam途径获取sessionId，再通过sessionId去sessionDAO(一般为缓存)中获取
     */
    ...
    protected void doFilterInternal(ServletRequest servletRequest, ServletResponse servletResponse, final FilterChain chain) throws ServletException, IOException
    {
        ...
        try
        {
            // 封装Request，根据isHttpSessions()的不同，获取session的方式不同
            final ServletRequest request = prepareServletRequest(servletRequest, servletResponse, chain);
            // 封装response，只有isHttpSessions()为false表示不支持httpSession才封装，封装后的机制是在重定向时会将sessionId拼接在url中
