...
public class AllocateMappedFileService extends ServiceThread
{
    ...
    public MappedFile putRequestAndReturnMappedFile(String nextFilePath, String nextNextFilePath, int fileSize)
    {
        ...
        if (nextPutOK)
        {
            ...
            boolean offerOK = this.requestQueue.offer(nextReq);//添加创建请求到队列，由线程进行异步创建
            if (!offerOK)
            {
        ...
        try
        {
            if (result != null)
            {
                ...
                {
                    // waitOK 为 true，表明 mappedFile 已创建好
                    this.requestTable.remove(nextFilePath);
                    //这里有个疑问，上面创建了两个，这里只返回第一个，第二个如何被外部引用？？？
    ...
    public void run()
    {
        ...
        //一直循环的创建文件,当队列为空时会阻塞在PriorityBlockingQueue的take方法上
    ...
    private boolean mmapOperation()
    {
        ...
        try
        {
            ...
            if (req.getMappedFile() == null)
            {
                ...
                // 如果文件大小大于 1G，并且开启 warmMapedFileEnable 预热开关（默认 false）
