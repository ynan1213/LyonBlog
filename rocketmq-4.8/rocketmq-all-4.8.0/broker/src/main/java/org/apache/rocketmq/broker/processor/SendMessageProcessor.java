...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public boolean rejectRequest() {
        return this.brokerController.getMessageStore().isOSPageCacheBusy() || this.brokerController.getMessageStore()
            .isTransientStorePoolDeficient();
    }

    private CompletableFuture<RemotingCommand> asyncConsumerSendMsgBack(ChannelHandlerContext ctx, RemotingCommand request)
        throws RemotingCommandException {
        ...
        // 获取消息订阅配置信息
        // 消费组订阅信息存储在 subscriptionGroup.json 中，默认情况下BrokerConfig.autoCreateSubscriptionGroup为true
        // 表示第一次使用消费组配置信息时如果不存在，则自动创建一个，如果为false，则只能通过客户端命令创建后修改其参数
        ...
        if (null == subscriptionGroupConfig) {
            ...
            response.setRemark("subscription group not exist, " + requestHeader.getGroup() + " " + FAQUrl
        ...
        // 随机选择一个队列，重试主题的队列数量本来默认就为1
        int queueIdInt = Math.abs(this.random.nextInt() % 99999999) % subscriptionGroupConfig.getRetryQueueNums();
        int topicSysFlag = 0;
        if (requestHeader.isUnitMode()) {
            topicSysFlag = TopicSysFlag.buildSysFlag(false, true);
        }
        // 查找或者创建 %RETRY% + groupName 的配置对象，以前有就直接返回，通常consumer首次发送心跳给broker，broker就会创建 %RETRY% + groupName 的topic
        ...
        // 找到原消息，因为consumer返回回来只有主题和偏移量，没有消息体
        ...
        // 如果消息重试消费次数超过了阈值，或者延迟级别小于0，再次改变消息主题为 %DLQ% + groupName
        // %DLQ% 的主题权限为只写，说明消息一旦进入到DLQ队列中，将无法进行消费了，需要人工干预
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes || delayLevel < 0) {
            newTopic = MixAll.getDLQTopic(requestHeader.getGroup());
            queueIdInt = Math.abs(this.random.nextInt() % 99999999) % DLQ_NUMS_PER_GROUP;

            topicConfig = this.brokerController.getTopicConfigManager()
                .createTopicInSendMessageBackMethod(newTopic, DLQ_NUMS_PER_GROUP, PermName.PERM_WRITE, 0);
            if (null == topicConfig) {
                response.setCode(ResponseCode.SYSTEM_ERROR);
                response.setRemark("topic[" + newTopic + "] not exist");
                return CompletableFuture.completedFuture(response);
        ...
        // 创建一条新的消息，存入commitlog中，并不会去更新原先消息，而是将原先的主题、消息ID存入属性中
    ...
    private CompletableFuture<RemotingCommand> handlePutMessageResultFuture(CompletableFuture<PutMessageResult> putMessageResult,
        ...
        return putMessageResult
            .thenApply((r) -> handlePutMessageResult(r, response, request, msgInner, responseHeader, sendMessageContext, ctx, queueIdInt));
    }
    ...
    private boolean handleRetryAndDLQ(SendMessageRequestHeader requestHeader, RemotingCommand response,
        ...
        if (null != newTopic && newTopic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {
            ...
            SubscriptionGroupConfig subscriptionGroupConfig = this.brokerController.getSubscriptionGroupManager()
                .findSubscriptionGroupConfig(groupName);
            if (null == subscriptionGroupConfig) {
                response.setCode(ResponseCode.SUBSCRIPTION_GROUP_NOT_EXIST);
                response.setRemark(
    ...
    private RemotingCommand handlePutMessageResult(PutMessageResult putMessageResult, RemotingCommand response,
        RemotingCommand request, MessageExt msg, SendMessageResponseHeader responseHeader, SendMessageContext sendMessageContext,
        ...
        switch (putMessageResult.getPutMessageStatus()) {
            ...
            case SERVICE_NOT_AVAILABLE:
                ...
                response.setRemark("service not available now. It may be caused by one of the following reasons: " +
                    "the broker's disk is full [" + diskUtil()
        ...
        if (sendOK) {
            ...
            this.brokerController.getBrokerStatsManager()
                .incTopicPutSize(msg.getTopic(), putMessageResult.getAppendMessageResult().getWroteBytes());
    ...
    private CompletableFuture<RemotingCommand> asyncSendBatchMessage(ChannelHandlerContext ctx, RemotingCommand request,
        ...
        return handlePutMessageResultFuture(putMessageResult, response, request, messageExtBatch, responseHeader, mqtraceContext, ctx,
            queueIdInt);
    }
