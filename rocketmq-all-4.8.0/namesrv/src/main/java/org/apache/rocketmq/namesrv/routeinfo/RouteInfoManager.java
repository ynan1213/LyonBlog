...
public class RouteInfoManager
{
    ...
    // key:topic，记录一个主题的队列分布在哪些Broker上，每个Broker上存在该主题的队列个数
    private final HashMap<String/* topic */, List<QueueData>> topicQueueTable;

    // key:brokerName，存储Broker基础信息，包含brokerName、所属集群名称、主备Broker地址
    // 注意：这里有个坑，如果不同的集群有相同名称的broker，这里是无法进行区分的，因为心跳是30s，所以会不停地互相覆盖
    // 疑问：分集群有什么意义吗？ 为了运维方便？？？
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;

    // key:clusterName，value:brokerName，一个集群下有哪些broker，注意，主从broker使用同一个name
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;

    // key:brokerAddr，存储Broker的状态信息，NameServer每次收到心跳包会替换该信息
            ...
            final Channel channel)
    {
        ...
        try
        {
            try
            {
                ...
                // 是个set集合，所以同一个集群下broker如果name相同，就会被覆盖，主从通过brokerId进行分
    ...
    /**
     * producer定时获取topic的TopicRouteData信息
     */
