...
public abstract class PathMatchingFilter extends AdviceFilter implements PathConfigProcessor
{
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception
    {
        ...
        // 每个过滤器在初始化的时候会将所有自己需要拦截的url全都存储在appliedPaths中，如果appliedPaths中没有，说明无需拦截
        // key是需要拦截的url，value为 /test.jsp" = roles[admin,user] 中[]里面的内容，如果没有配置，value则为空，但是key不会为null
        ...
        // 上面如果没有匹配到，说明该过滤器不进行拦截，返回true，AdviceFilter的preHandle也就返回true，继续执行后续过滤器
        return true;
    }
