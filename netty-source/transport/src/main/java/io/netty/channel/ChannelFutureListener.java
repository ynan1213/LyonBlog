 ...
 *
 * 一旦 ChannelFuture#addListener(GenericFutureListener)添加了监听器, 异步操作的结果就会被通知到
 *
 ...
 * thread.
 * operationComplete(Future)方法, 直接会被 观察者调用
 *
 * Therefore, performing a time consuming task or a blocking operation
 * in the handler method can cause an unexpected pause during I/O.  If you need
 * to perform a blocking operation on I/O completion, try to execute the
 * operation in a different thread using a thread pool.
 *
 * 因此完成一个耗时的任务或者阻塞的任务在IO线程中,会带来不可预期的结果, 推荐把他们放到一个业务线程池中
 */
