...
public class PooledByteBufAllocator extends AbstractByteBufAllocator implements ByteBufAllocatorMetricProvider {
    ...
    private static final int DEFAULT_TINY_CACHE_SIZE; // 默认512
    private static final int DEFAULT_SMALL_CACHE_SIZE; // 默认256
    private static final int DEFAULT_NORMAL_CACHE_SIZE; // 默认64
    ...
    private final int tinyCacheSize; // 代表tiny类型的ByteBuf能缓存多少个
    private final int smallCacheSize; // 代表small类型的ByteBuf能缓存多少个
    private final int normalCacheSize; // 代表normal类型的ByteBuf能缓存多少个
    ...
    protected ByteBuf newDirectBuffer(int initialCapacity, int maxCapacity) {
        // 获取线程局部缓存 , 它里面有维护着两大块内存,一块是堆内的内存,一块是堆外的内存
        PoolThreadCache cache = threadCache.get();
