...
public class MQClientAPIImpl {
    ...
    public void createPlainAccessConfig(final String addr, final PlainAccessConfig plainAccessConfig, final long timeoutMillis)
        throws RemotingException, MQBrokerException, InterruptedException, MQClientException {
    ...
    public ClusterAclVersionInfo getBrokerClusterAclInfo(final String addr, final long timeoutMillis)
        ...
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                GetBrokerAclConfigResponseHeader responseHeader = (GetBrokerAclConfigResponseHeader) response
                    .decodeCommandCustomHeader(GetBrokerAclConfigResponseHeader.class);
    ...
    public AclConfig getBrokerClusterConfig(final String addr, final long timeoutMillis)
        throws RemotingCommandException, InterruptedException, RemotingTimeoutException, RemotingSendRequestException, RemotingConnectException, MQBrokerException {
        ...
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                if (response.getBody() != null) {
                    GetBrokerClusterAclConfigResponseBody body = GetBrokerClusterAclConfigResponseBody
                        .decode(response.getBody(), GetBrokerClusterAclConfigResponseBody.class);
    ...
    public SendResult sendMessage(
        ...
        switch (communicationMode) {
            ...
            case ASYNC:
                ...
                // }
                this.sendMessageAsync(addr, brokerName, msg, timeoutMillis - costTimeAsync, request, sendCallback, topicPublishInfo,
                    instance, retryTimesWhenSendFailed, times, context, producer);
                // 异步方式的结果在 sendCallback 回调方法里处理
                return null;
            case SYNC:
    ...
    private void sendMessageAsync(
        ...
        this.remotingClient.invokeAsync(addr, request, timeoutMillis, new InvokeCallback() {
            ...
            public void operationComplete(ResponseFuture responseFuture) {
                ...
                // 情况一：回调函数为null 且 response 不为null
                // 因为这个方法是异步的，没有 sendCallback，如何对结果 SendResult 进行确认呢？
                ...
                // 情况二：回调函数不为null 且 response 不为null
                if (response != null) {
                    try {
                        SendResult sendResult = MQClientAPIImpl.this.processSendResponse(brokerName, msg, response, addr);
                        ...
                        try {
                            // 执行回调，一般是我们自定义在 DefaultMQProducer.send(msg, new SendCallback() {})这个 SendCallback对象
                    ...
                    // 情况三：response 为null，说明没有接收到broker的返回消息，可能是发送失败，也有可能是等待响应超时
                    producer.updateFaultItem(brokerName, System.currentTimeMillis() - responseFuture.getBeginTimestamp(), true);
    ...
    private void onExceptionImpl(final String brokerName,
        ...
        if (needRetry && tmp <= timesTotal) { // 有重试机会
            ...
            try {
                ...
                // 执行异步发送
                sendMessageAsync(addr, retryBrokerName, msg, timeoutMillis, request, sendCallback, topicPublishInfo, instance, timesTotal,
                    curTimes, context, producer);

            // 注意：下面的这些异常是当前线程执行异步发送时抛出来的，
            } catch (InterruptedException e1) {
                onExceptionImpl(retryBrokerName, msg, timeoutMillis, request, sendCallback, topicPublishInfo, instance, timesTotal,
                    curTimes, e1, context, false, producer);
            } catch (RemotingConnectException e1) {
                producer.updateFaultItem(brokerName, 3000, true);
                onExceptionImpl(retryBrokerName, msg, timeoutMillis, request, sendCallback, topicPublishInfo, instance, timesTotal,
                    curTimes, e1, context, true, producer);
            ...
            try {
                // 执行回调的 onException 方法，一般是我们自定义的
    ...
    /**
     * 处理 sendMessage 后返回的 RemotingCommand，包装成 SendResult
     * @param brokerName
     * @param msg 发送的Message
     * @param response Broker的返回
     * @param addr Broker的addr
     */
    private SendResult processSendResponse(final String brokerName, final Message msg, final RemotingCommand response, final String addr)
        ...
        // responseHeader.getQueueId() 返回的是实际发送到了Broker中的那个queueId
                ...
                if (response != null) {
                    ...
                    if (!responseFuture.isSendRequestOK()) {
                        ...
                        pullCallback.onException(new MQClientException(
                            "wait response from " + addr + " timeout :" + responseFuture.getTimeoutMillis() + "ms" + ". Request: "
                                + request, responseFuture.getCause()));
                    } else {
                        pullCallback.onException(new MQClientException(
                            "unknown reason. addr: " + addr + ", timeoutMillis: " + timeoutMillis + ". Request: " + request,
                            responseFuture.getCause()));
                    }
                }
            }
    ...
    // 同步拉取
    private PullResult pullMessageSync(final String addr, final RemotingCommand request, final long timeoutMillis)
    ...
    public List<String> getConsumerIdListByGroup(final String addr, final String consumerGroup, final long timeoutMillis)
        ...
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                if (response.getBody() != null) {
                    GetConsumerListByGroupResponseBody body = GetConsumerListByGroupResponseBody
                        .decode(response.getBody(), GetConsumerListByGroupResponseBody.class);
    ...
    public void queryMessage(
        ...
        this.remotingClient
            .invokeAsync(MixAll.brokerVIPChannel(this.clientConfig.isVipChannelEnabled(), addr), request, timeoutMillis, invokeCallback);
    }
