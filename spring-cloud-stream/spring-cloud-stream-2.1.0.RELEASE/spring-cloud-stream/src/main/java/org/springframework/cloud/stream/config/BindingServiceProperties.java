...
public class BindingServiceProperties implements ApplicationContextAware, InitializingBean {
	...
	public BindingProperties getBindingProperties(String bindingName) {
		// 如果没有 bindingName 的配置项，这里会初始化默认的配置项
		this.bindIfNecessary(bindingName);
		BindingProperties bindingProperties = this.bindings.get(bindingName);
		// 如果未配置 destination，默认就是bindingName，也就是@Input或者@Output注解的value值或者是方法名
	...
	private void bindToDefault(String binding) {
		...
		Binder binder = new Binder(
			...
			IntegrationUtils.getConversionService(applicationContext.getBeanFactory()),
			null);
