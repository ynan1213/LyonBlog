...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager implements ResourceTransactionManager, InitializingBean {
	...
	/**
	 * 该方法是对父类抽象方法的实现
	 * DataSourceTransactionManager 对事物的抽象就是一个数据库连接，用 DataSourceTransactionObject 表示
	 */
	@Override
	protected Object doGetTransaction() {
		...
		// 真正的连接由 TransactionSynchronizationManager 保存到 threadLocal中
		// 每个线程在threadLocal中保存的是一个map，key就是dataSource数据源，value就是一个connect
		// 也就是一个线程可以同时从不同的数据源获取到连接从而可以同时拥有多个事物
		// 但是一个数据源同时只能有一个事物，如果new和nested的情况下是怎么解决的呢？？ 答：挂起原先的事物再清空线程私有域
		// 从事物同步管理器获取连接，如果没有外层事物，则为空，内部是从ThreadLocal获取
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			// 给 connection 设置 readOnly 和 隔离级别
	...
	protected Object doSuspend(Object transaction) {
		// 但是不明白为什么先要将 txObject.setConnectionHolder(null) ？？？？？
		DataSourceTransactionObject txObject = (DataSourceTransactionObject) transaction;
	...
	protected void doSetRollbackOnly(DefaultTransactionStatus status) {
		...
		// 给事务标记为回滚，这个标记是一种“全局的标记”，因为隶属于同一个物理事务都能读到同一个ConnectionHolder
