 ...
 * </ul> 他自己通常不会提供太多的方法,但是你可以实现 in out bound这两个类去处理出栈,入栈的操作
 * </p>
 * <p>
 ...
 * </ul>  为了图方便可以选择上面提供好的适配器, 去处理 in out 的 IO操作
 * </p>
 * <p>
 ...
 * 使用 handler我们可以做到, 向上游传递事件,或者向下游传递事件,或者动态的修改管道, 储存信息,用于处理特定的事件的程序
 * <h3>State management</h3>
 *
 * A {@link ChannelHandler} often needs to store some stateful information.
 * The simplest and recommended approach is to use member variables:
 * <pre>
 *     下面是一个例子, handler通常用于存储一些有状态的信息
 ...
 * one connection,
 * 因为handler实例中存储了一个 专注于建立连接的有状态的变量
 * you have to create a new handler instance for each new
 * channel to avoid a race condition where a unauthenticated client can get
 * the confidential information:
 * 所以你必须为每一个新的channel 创建一个新的handler, 用于处理未授权的client去获取机密的信息
 * <pre>
 ...
 * <pre>
 *     尽管推荐使用一系列的存储有状态的数据的handler, 但是鉴于其他原因, 如果你不想使用instances实例,
 *     在这种情况下, 你可以使用 AttributeKey , 它被 HandlerContext提供
 * public interface Message {
 *     // your methods here
 * }
 *
 ...
 * a race condition.
 * <p>  @Sharable 意味着你可以把同一个handler 添加在不同的pipeline, 否则不允许
...
public interface ChannelHandler {
     ...
     *
     * 当handler被添加到真实的上下文(pipeline)中触发的回调
     */
     ...
     * <a href="http://www.javaconcurrencyinpractice.com/annotations/doc/">the JCIP annotations</a>.
     *
     *  标记这个channelhandler可以被多个pipeline添加
     */
    @Inherited
    @Documented
