...
final class PostProcessorRegistrationDelegate
{
	...
	public static void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory, List<BeanFactoryPostProcessor> beanFactoryPostProcessors)
	{
		...
		if (beanFactory instanceof BeanDefinitionRegistry)
		{
			...
			// 第二步：实现了 Ordered 接口的
			// 因为第一步可能向容器注册了新的 BeanFactoryPostProcessor，再查找一遍；
			...
			// 第三步：未实现 Ordered和PriorityOrdered 接口的
			// 因为第二步可能向容器注册了新的 BeanFactoryPostProcessor，再查找一遍；
			...
			// 第四步：上面三步执行的是 BeanDefinitionRegistryPostProcessor 接口的方法，这里执行 BeanFactoryPostProcessor 的方法
		...
		// 上面四步之后，有可能向容器注册了新的 BeanFactoryPostProcessor，再查找一遍；
	...
	{
		// 注意：此处只会拿到Bean的定义信息~~~~
		// 已经被实例化的Bean最终都会调用`beanFactory.addBeanPostProcessor`而缓存在AbstractBeanFactory的beanPostProcessors里，它是个CopyOnWriteArrayList
		...
		// +1 的目的是手动将BeanPostProcessorChecker添加进去了，也就是说容器中一共会存在 beanProcessorTargetCount 个 BeanPostProcessor
		// BeanPostProcessorChecker目的：
		// 		在bean初始化的时候如果检测到bean工厂中已实例化的 BeanPostProcessor 个数小于 beanProcessorTargetCount，并且当前初始化的bean不是
		//		BeanPostProcessor类型，说明当前bean在BeanPostProcessor之前就被初始化了，给info提示
