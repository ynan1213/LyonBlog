...
public class DefaultWebSecurityManager extends DefaultSecurityManager implements WebSecurityManager {
    ...
    /**
     * @since 1.0
     * `isHttpSessionMode()`是由SessionManager 决定，具体逻辑如是：
     *  1.如果创建`SecurityManager`对象时未自定义`SessionManager`，DefaultWebSecurityManager默认创建ServletContainerSessionManager类型，
     *    该类型的作用机制是`session`对象交给`Servlet`容器管理，调用`subject.getSession()`和`ShiroHttpServletRequest.getSession()`方法
     *    最终调用的是原生`request.getSession(false)`，并且`isServletContainerSessions()`返回**true**；
     *  2.如果自定义`SessionManager`类型，一般均为`DefaultWebSessionManager`类型，此时`isServletContainerSessions()`字段为**false**
     */
