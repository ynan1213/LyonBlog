...
public class DispatcherServlet extends FrameworkServlet
{
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			try
			{
				// 检查请求是否是multipart（即文件上传
				processedRequest = checkMultipart(request);
				multipartRequestParsed = (processedRequest != request);

				// 通过handermapping 映射获取 HandlerExecutionChain（包括了interceptor的前置和后置方法）
				...
				if (isGet || "HEAD".equals(method))
				{
					...
					// RequestMappingHandlerAdapter 直接返回 -1，不走缓存
			...
			/**
			 * 处理handler执行结果，不管结果是ModelAndView还是异常，最终都被处理为ModelAndView
			 * 		1、如果有异常就处理异常
			 * 		2、视图渲染，核心方法
			 * 		3、执行拦截器最终的afterCompletion方法
			 */
									   ...
									   @Nullable Exception exception) throws Exception
	{
		...
		{
			// 什么情况下 mv 为null呢？ @RepsonseBody 或者 @ResponseStatus
			if (logger.isTraceEnabled())
			{
