...
public abstract class AdviceFilter extends OncePerRequestFilter
{
    ...
    public void doFilterInternal(ServletRequest request, ServletResponse response, FilterChain chain) throws ServletException, IOException
    {
        ...
        {
            /**
             * 决定过滤器链是否向后执行，preHandle 由子类进行重写
             * ①：preHandle：PathMatchingFilter：进行url与filter的匹配，没有匹配的也会返回true，有匹配的进入onPreHandle方法，由子类重写
             * ②：onPreHandle：AccessControlFilter，提供 isAccessAllowed || onAccessDenied 方法
             *      isAccessAllowed返回true，onPreHandle直接返回true
             *      isAccessAllowed返回false，进入onAccessDenied处理，比如跳转、给出错误提示
             */
            ...
            // 无论 preHandle 返回true还是false，这里都会执行
