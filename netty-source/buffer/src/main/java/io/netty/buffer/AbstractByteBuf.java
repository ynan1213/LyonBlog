...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    int readerIndex;//读指针
    int writerIndex;//写指针
    private int markedReaderIndex;//保存读指针
    private int markedWriterIndex;//保存写指针
    private int maxCapacity;//最大分配容量
    ...
    public ByteBuf setIndex(int readerIndex, int writerIndex) {
        if (readerIndex < 0 || readerIndex > writerIndex || writerIndex > capacity()) {
            throw new IndexOutOfBoundsException(String.format(
                    "readerIndex: %d, writerIndex: %d (expected: 0 <= readerIndex <= writerIndex <= capacity(%d))", readerIndex, writerIndex, capacity()));
        }
    ...
    final void ensureWritable0(int minWritableBytes) {
        ensureAccessible();
        // writableBytes 是 capacity - writeIndex，意思是空闲的容量，如果要写的长度小于等于空闲容量，直接返回不用扩容
        if (minWritableBytes <= writableBytes()) {
            return;
        }
        // 需要写入的长度+写指针必须要小于最大分配的内存, 否则报错，注意这里最大分配内存不带表当前内存, 而是byteBuf所能分配的最大内存
        ...
        // 到这里说明需要扩容，计算新的容量
        // writerIndex + minWritableBytes 代表扩容后的最小容量, maxCapacity为byteBuf所能分配的最大内存
        int newCapacity = alloc().calculateNewCapacity(writerIndex + minWritableBytes, maxCapacity);

        // Adjust to the new capacity. 扩容
    ...
    public ByteBuf writeBytes(ByteBuf src, int srcIndex, int length) {
        // 确保可写容量充足
        ensureWritable(length);
        // 这里的参数的意思是从当前byteBuf的writerIndex节点开始写入, 将源缓冲区src的读指针位置, 写lenght个字节,
        // 这里的方法中AbstractByteBuf类并没有提供实现, 因为不同类型的BtyeBuf实现的方式是不一样的, 所以这里交给了子类去实现
        setBytes(writerIndex, src, srcIndex, length);
        // 最后将写指针后移length个字节
        writerIndex += length;
        return this;
