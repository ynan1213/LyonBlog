...
public class ReflectiveFeign extends Feign {
	...
	private static class BuildTemplateByResolvingArgs implements RequestTemplate.Factory {
		...
		public RequestTemplate create(Object[] argv) {
			...
			// 将md中原本的RequestTemplate取出，创建新的RequestTemplate，以供修改
			...
			// 什么样的参数会被记录在 indexToName集合中？ 目前发现的是被 @RequestParam 和 @PathVariable 注解的参数
			// 这些参数最终会被拼接在url后面，如果是Object对象如何拼接？ 调用toString()方法然后urlEncode
			// 对于没有注解或者是被@RequestBody注解的参数，这里会被跳过，因为这些参数是被写入body中的
			...
			// BuildEncodedTemplateFromArgs 对象覆盖了该方法，它对父类 BuildTemplateByResolvingArgs 的功能进行了扩充：
			// 将没有注解或者是被@RequestBody注解的参数写入到 RequestTemplate 的请求体body中
			// 在父类 BuildTemplateByResolvingArgs 中，就是将参数拼接到url中
			RequestTemplate template = resolve(argv, mutable, varBuilder);

			// queryMapIndex，参数中有请求键值对？，这种用法没有见过
			...
			// headerMapIndex，参数中有请求头，这种用法也没有用过
	...
	private static class BuildEncodedTemplateFromArgs extends BuildTemplateByResolvingArgs {
		...
		protected RequestTemplate resolve(Object[] argv, RequestTemplate mutable, Map<String, Object> variables) {
			...
			try {
				// 一般实现是 SpringEncoder，内部逻辑就是将body对象序列化并封装到 RequestTemplate 的body中
