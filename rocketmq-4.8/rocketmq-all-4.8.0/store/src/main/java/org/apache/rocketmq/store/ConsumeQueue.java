...
public class ConsumeQueue
{
    ...
    // 文件大小，默认30w * 20字节 = 600w字节  约5.72M
    private final int mappedFileSize;

    // 当前 ConsumeQueue 中所记录的 CommitLog的最大物理偏移地址
    private long maxPhysicOffset = -1;

    // 当前 ConsumeQueue 中的最小逻辑偏移量？ 物理偏移量？
            ...
            if (mappedFile != null)
            {
                ...
                for (int i = 0; i < logicFileSize; i += CQ_STORE_UNIT_SIZE)
                {
                    ...
                    if (0 == i)
                    {
                        if (offset >= phyOffet)
                        {
                            // 若第一块消息单元中的物理偏移值 offset（第1至8个字节）大于或等于 phyOffet，则从MapedFile列表中删除该记录并从磁盘中删除物理文件
                            this.mappedFileQueue.deleteLastMappedFile();
                            break;
                        } else
                        {
                            // 若第一块消息单元中的物理偏移值 offset（第1至8个字节）小于 phyOffet，
                            int pos = i + CQ_STORE_UNIT_SIZE;
                            mappedFile.setWrotePosition(pos);
                            mappedFile.setCommittedPosition(pos);// 不知道为什么 CommittedPosition 和 FlushedPosition 也要更新
                    ...
                    {
                        // 不断地更新MappedFile文件wrotePostion和CommittedPosition两个变量为解析到的数据块位置；
                        // 直到解析到数据块的大小为空或者物理偏移值大于了 phyOffet 为止。
                        if (offset >= 0 && size > 0)
                        {
                            if (offset >= phyOffet)
                            {
    ...
    {
        // 获取 MapedFileQueue 队列中第一个MapedFile对象
        MappedFile mappedFile = this.mappedFileQueue.getFirstMappedFile();
        long minExtAddr = 1;
        if (mappedFile != null)
        {
            // 然后调用selectMapedBuffer(0)方法从第0位开始读取所有数据；
    ...
    public SelectMappedBufferResult getIndexBuffer(final long startIndex)
    {
        ...
        if (offset >= this.getMinLogicOffset())
        {
            ...
            if (mappedFile != null)
            {
                // 返回 offset % mappedFileSize 到最大可读之间的数据
